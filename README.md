...
| **取坐标 / 截图** | 三个命令行小脚本 | 界面内置坐标拾取器（跟随鼠标显示坐标）与区域框选截图 |
...
| 任务设置 | 循环开关与轮数、开始倒计时、目标窗口标题与「不在前台时等待」、左上角急停、空跑、随机抖动、动作最小间隔 |
...
# 指定目标窗口，窗口不在前台则等待
python -m autoclicker.cli profiles/example.json --target-window "记事本"
...
│   ├── cli.py                  # 命令行接口
│   ├── gui.py                  # tkinter 图形界面 + 坐标拾取器 + 区域截图
│   ├── engine.py               # 执行引擎（中止/暂停/空跑/失败保护）
